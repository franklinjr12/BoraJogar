...
export function DashboardPage() {
  ...
  return (
    <main className="shell">
      ...
      <h1>What’s next?</h1>
      <p className="lead">Important game actions and commitments, ready at a glance.</p>
      ...
      {!error && confirmed.length === 0 && open.length === 0 && (
        <section className="card">
          <h2>No upcoming games.</h2>
          <p>Set availability or create a game to start planning.</p>
          <div className="actions">
            <Link className="button" to="/games/new">
              Create a game
            </Link>
      ...
      {confirmed.length > 0 && (
        <section className="dashboard-section">
          <h2>Upcoming confirmed games</h2>
          <div className="game-list">{confirmed.map(gameCard)}</div>
        </section>
      )}
      {open.length > 0 && (
        <section className="dashboard-section">
          <h2>Games with open slots</h2>
          <div className="game-list">{open.map(gameCard)}</div>
        </section>
      )}
      {!error && confirmed.length === 0 && open.length > 0 && (
        <section className="dashboard-section">
          ...
          <p className="card">
            Your schedule can improve matches.{' '}
            <Link className="text-link" to="/availability">
              Review availability
            </Link>
          </p>

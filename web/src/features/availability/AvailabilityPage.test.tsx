...
describe('AvailabilityPage', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      const body = url.includes('preferred-areas') ? [{ id: 'area-1', label: 'Near home', active: true }] : [];
  ...
  it('shows weekly editor and location requirement', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('heading', { name: /weekly availability/i })).toBeInTheDocument());

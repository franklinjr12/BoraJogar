...
func TestValidateProfileInputNormalizesAndGatesMatchmaking(t *testing.T) {
	...
	if got.DisplayName != "Ana" || *got.Bio != "beach volleyball fan" || *got.ActiveForMatchmaking { t.Fatalf("normalized profile = %+v", got) }
	...
	if err != nil || !*got.ActiveForMatchmaking { t.Fatalf("completed profile should activate matchmaking: %+v, %v", got, err) }
}
...
func TestValidateProfileInputRejectsInvalidValues(t *testing.T) {
	...
	for _, input := range cases { if _, err := validateProfileInput(input, true); err == nil { t.Fatalf("expected invalid profile rejection: %+v", input) } }
}

func TestValidateProgress(t *testing.T) {
	if err := validateProgress(progress{CurrentStep: 8, CompletedSteps: []int{0, 4, 8}}); err != nil { t.Fatal(err) }
	if err := validateProgress(progress{CurrentStep: 9}); err == nil { t.Fatal("expected current step rejection") }
	if err := validateProgress(progress{CurrentStep: 1, CompletedSteps: []int{-1}}); err == nil { t.Fatal("expected completed step rejection") }
}
